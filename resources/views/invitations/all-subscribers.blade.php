                            ...
                            <table class="table table-striped projects">
                                ...
                                <tbody>
                                    @foreach($allSubscribers as $subscriber)
                                        <tr>
                                            <td>
                                                {{ $subscriber->name }} {{ $subscriber->first_name }}
                                                <br/>
                                                <small>
                                                    <b>Subscription date</b> : {{$subscriber->created_at}}
                                                </small>
                                                <br/>
                                                @if($subscriber->activeUser === 1 && $subscriber->invoice_paid === 1)
                                                    <small class="badge badge-success">Paid</small>
                                                @endif
                                            </td>
                                            <td>
                                                <img class="table-avatar img-circle elevation-1" width="32" height="32"
                                                src="{{ asset('public/storage/'. $subscriber->profile_photo_path) }}">
                                            </td>
